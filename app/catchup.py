...
from app.notifier import send_telegram_message, send_video_notification
from app.rate_limit import exclusive_job_wait, wait_between_channels
    ...
    with exclusive_job_wait("catch-up push", timeout=CATCHUP_LOCK_TIMEOUT) as acquired:
        ...
        if not failures:
            if len(channel_ids) > 1:
                send_telegram_message(
                    user_id,
                    f"✅ 已完成 {len(channel_ids)} 個頻道的最新影片推播。",
                )
            return
        ...
        send_telegram_message(user_id, header + "\n\n".join(lines))
