...
#### ngrok（建議：免買網域、NAS 不開公網 port）

ngrok 用**出站連線**把本機 `127.0.0.1:8080` 變成 HTTPS，私人相簿等服務不會對外開放。

**1. 註冊並取得 token / 免費網域**

1. 開啟 [ngrok Dashboard](https://dashboard.ngrok.com/) 註冊並登入
2. **Your Authtoken** → 複製 token（[Getting Started](https://dashboard.ngrok.com/get-started/your-authtoken)）
3. 左側 **Domains** → **New Domain**（或 Create Free Domain）→ 領一組免費靜態網域，例如 `your-name.ngrok-free.app`  
   （務必用靜態網域，重開容器後網址才不會變）
...
**3. 啟動 QueryTube + ngrok**

```bash
docker compose --profile line up -d --build
```

- `querytube`：摘要服務（webhook 聽 `127.0.0.1:8080`）
- `ngrok`：僅在 `--profile line` 時啟動，轉發到本機 8080
...
應看到 `LINE bot started` / `Starting LINE webhook`，以及 ngrok `started tunnel` 指向你的網域。
...
# 瀏覽器或外網：
curl -s https://your-name.ngrok-free.app/health
```
...
```text
https://your-name.ngrok-free.app/line/webhook
```

在 LINE Developers → Messaging API → 填入後 **Verify**，並將 **Use webhook** 設為 Enabled。把官方帳號拉進群組後貼 YouTube 連結即可測試。

未設定 LINE 憑證時，不必開 `--profile line`；Telegram 行為不變。
...
| `NGROK_AUTHTOKEN` | — | （可選）ngrok authtoken；搭配 `--profile line` |
| `NGROK_DOMAIN` | — | （可選）ngrok 免費靜態網域，如 `xxx.ngrok-free.app` |
